...
const { unreadMsg } = storeToRefs(notificationStore);

const isNoticeVisible = ref(false);

function onPopupVisibleChange(visible, context) {
...
<template>
  ...
  >
    <template #content>
      <div class="header-msg">
        ...
        <div v-else class="empty-list">
          <img src="@/assets/images/no-message.png" alt="空" />
          <p>暂无通知</p>
        </div>
        <div class="header-msg-bottom">
          <t-button v-if="unreadMsg.length > 0" class="header-msg-bottom-link" variant="text" @click="goDetail"
            >查看全部</t-button
          >
        </div>
      </div>
    ...
    <t-badge :count="unreadMsg.length" :offset="[15, 6]">
      <t-button shape="square" variant="text" @click="isNoticeVisible = true">
        <t-icon name="mail" />
      </t-button>
    </t-badge>
  </t-popup>
